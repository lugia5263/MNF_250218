...
public class StageSelectTalkManager : MonoBehaviour
{
    ...
    //TODO: setActive가 켜지면 최초로 scene이름과 동일한 json을 받아오는 함수
    void Start()
    {
    ...
    //TODO: 범용성을 위해 매개변수로 변환함, 버튼 두번누르면 대화가 바뀜
    public void init(string talkJsonName)
    {

        shopDataDict.Clear();
        string SceneName = SceneManager.GetActiveScene().name;

        TextAsset jsonFile = Resources.Load<TextAsset>($"{talkJsonName}"); //파일명으로 불러오기
        Debug.Log($"불러와진 파일명은 {talkJsonName}입니다. 변환되었습니다");
      //TextAsset jsonFile = Resources.Load<TextAsset>(SceneName); 씬네임불러오기
